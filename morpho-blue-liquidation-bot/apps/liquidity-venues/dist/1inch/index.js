...
export class OneInch {
    ...
    supportsRoute(encoder, src, dst) {
        if (src === dst)
            return false;
        if (!supportedNetworks.includes(encoder.client.chain.id))
            return false;
    ...
    async convert(encoder, toConvert) {
        try {
            ...
            throw new Error(`(1inch) Error fetching swap response: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    getSwapApiPath = (chainId) => `/swap/v6.1/${chainId}/swap`;
