import { FEE_TIERS, DEFAULT_FACTORY_ADDRESS, specificFactoryAddresses, MAX_SQRT_RATIO, MIN_SQRT_RATIO, } from "@morpho-blue-liquidation-bot/config";
import { executorAbi } from "executooor-viem";
import { encodeAbiParameters, encodeFunctionData, erc20Abi, fromHex, zeroAddress, } from "viem";
import { readContract } from "viem/actions";
import { uniswapV3FactoryAbi, uniswapV3PoolAbi } from "../abis/uniswapV3";
export class UniswapV3Venue {
    pools = {};
    ...
    async convert(encoder, toConvert) {
        ...
        try {
            ...
            const encodedContext = `0x${0n.toString(16).padStart(24, "0") + zeroAddress.substring(2)}`;
            const callbacks = [
                encodeFunctionData({
                    abi: executorAbi,
            ...
            encoder.pushCall(biggestPool, 0n, encodeFunctionData({
                abi: uniswapV3PoolAbi,
                functionName: "swap",
                args: [
                    encoder.address,
            ...
            throw new Error(`(UniswapV3) Error swapping: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    ...
    async fetchPools(encoder, src, dst) {
        const factoryAddress = specificFactoryAddresses[encoder.client.chain.id] ?? DEFAULT_FACTORY_ADDRESS;
        try {
            const newPools = (await Promise.all(FEE_TIERS.map(async (fee) => readContract(encoder.client, {
            ...
            throw new Error(`(UniswapV3) Error fetching pools: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
}
