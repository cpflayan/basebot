...
export class MidasVenue {
    ...
    async convert(encoder, toConvert) {
        ...
        const redemptionParams = await this.getRedemptionParams(redemptionVault, tokenOut, srcAmount, encoder);
        ...
        encoder.pushCall(redemptionVault, 0n, encodeFunctionData({
        ...
        return {
            src: tokenOut,
            srcAmount: this._convertFromBase18(amountTokenOutWithoutFee, redemptionParams.tokenOutDecimals),
            dst: dst,
        };
    }
    isMidasToken(token, chainId) {
        return Object.keys(midasConfigs[chainId] ?? {}).some((tokenAddress) => tokenAddress === token);
    }
    postRedeemToken(token, chainId, dst) {
        ...
        if (!firstRedeemToken)
            throw new Error(`(Midas) No redemption assets for token ${token}`);
        return redeemTokens.includes(dst) ? dst : firstRedeemToken;
    }
    redemptionVault(token, chainId) {
        const chainConfig = midasConfigs[chainId];
        if (!chainConfig?.[token])
            throw new Error(`(Midas) No config for token ${token} on chain ${chainId}`);
        return chainConfig[token].instantRedemptionVault;
    }
    previewRedeemInstant(params) {
        const feeData = this._calcAndValidateRedeem(params);
        if (!feeData)
            throw new Error(`(Midas) Error calculating and validating redeem for ${params.tokenOutConfig.dataFeed}`);
        if (!this._requireAndUpdateLimit(params, feeData.amountMTokenWithoutFee))
            throw new Error(`(Midas) Error validating redeem limit for ${params.tokenOutConfig.dataFeed}`);
        const usdData = this._convertMTokenToUsd(params, feeData.amountMTokenWithoutFee);
        if (!usdData)
            throw new Error(`(Midas) Error converting MToken to USD for ${params.tokenOutConfig.dataFeed}`);
        const tokenData = this._convertUsdToToken(params, usdData.amountUsd);
        if (!tokenData)
            throw new Error(`(Midas) Error converting USD to token for ${params.tokenOutConfig.dataFeed}`);
        return {
            amountTokenOutWithoutFee: this._truncate((feeData.amountMTokenWithoutFee * usdData.mTokenRate) / tokenData.tokenRate, params.tokenOutDecimals),
            feeAmount: feeData.feeAmount,
        };
    }
    _calcAndValidateRedeem(params) {
    ...
    _getFeeAmount(params) {
        if (params.waivedFeeRestriction)
            return 0n;
        const feePercent = MathLib.min(params.tokenOutConfig.fee + params.instantFee, ONE_HUNDRED_PERCENT);
        return (params.amountMTokenIn * feePercent) / ONE_HUNDRED_PERCENT;
    }
    _requireAndUpdateLimit(params, amount) {
        return params.dailyLimits + amount <= params.instantDailyLimit;
    }
    _convertMTokenToUsd(params, amount) {
    ...
    _convertUsdToToken(params, amountUsd) {
        if (amountUsd === 0n)
            throw new Error(`(Midas) Error converting USD to token for ${params.tokenOutConfig.dataFeed}`);
    ...
    // async methods
    async getRedemptionParams(vault, tokenOut, seizedCollateral, encoder) {
        ...
        try {
            const [minAmount, instantFee, instantDailyLimit, STABLECOIN_RATE, waivedFeeRestriction, dailyLimits, mTokenDataFeed, tokenOutConfig, tokenOutDecimals,] = await Promise.all([
            ...
            throw new Error(`(Midas) Error getting redemption params for ${vault} to ${tokenOut}: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    async getMidasRate(dataFeed, encoder) {
