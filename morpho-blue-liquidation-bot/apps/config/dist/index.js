...
export function chainConfig(chainId) {
    ...
    if (vaultWhitelist.length === 0 && additionalMarketsWhitelist.length === 0) {
        throw new Error(`Vault whitelist and additional markets whitelist both empty for chainId ${chainId}`);
    }
...
export function getSecrets(chainId, chain) {
    ...
    return {
        ...
        liquidationPrivateKey: liquidationPrivateKey,
    };
}
export * from "./chains";
export { chainConfigs, };
