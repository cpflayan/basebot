...
import { UniswapV3Venue, Erc4626, PendlePTVenue, } from "@morpho-blue-liquidation-bot/liquidity-venues";
...
import { MarketsFetchingCooldownMechanism, PositionLiquidationCooldownMechanism, } from "../../../src/utils/cooldownMechanisms.js";
...
describe("execute liquidation swapping on Uniswap V3", () => {
    ...
    encoderTest.sequential("should execute liquidation", async ({ encoder }) => {
        ...
        const bot = new LiquidationBot({
            ...
            pricers: [pricer],
            marketsFetchingCooldownMechanism: new MarketsFetchingCooldownMechanism(MARKETS_FETCHING_COOLDOWN_PERIOD),
    ...
    encoderTest.sequential("should not execute liquidation because no profit and alwaysRealizeBadDebt is false", async ({ encoder }) => {
        const pricer = new MorphoApi();
        const { client } = encoder;
    ...
    encoderTest.sequential("should execute liquidation even without profit and alwaysRealizeBadDebt is true", async ({ encoder }) => {
        const pricer = new MorphoApi();
        const { client } = encoder;
        const collateralAmount = parseUnits("0.0001", 8);
        const borrowAmount = parseUnits("5", 6);
...
describe("execute liquidation combining Pendle PT and 1inch liquidity venues", () => {
    ...
    pendleOneInchExecutionTest.sequential("should execute liquidation", async ({ encoder }) => {
        ...
        nock("https://api-v2.pendle.finance/core/")
            ...
            .get("/v2/sdk/1/markets/0xb6ac3d5da138918ac4e84441e924a20daa60dbdd/swap?receiver=0x2d493cde51adc74d4494b3dc146759cf32957a23&slippage=0.04&tokenIn=0xe6a934089bbee34f832060ce98848359883749b3&tokenOut=0x9d39a5de30e57443bff2a8307a4256c8797a3497&amountIn=5000000000000000000000")
            .reply(200, {
        ...
        expect(accountLoanTokenBalanceAfterLiquidation - accountLoanTokenBalanceBeforeLiquidation).toBeGreaterThan(0n);
    });
});
describe("position cooldown mechanism", () => {
    ...
    encoderTest.sequential("should skip liquidation when position is on cooldown", async ({ encoder }) => {
        const pricer = new MorphoApi();
        const { client } = encoder;
        const collateralAmount = parseUnits("0.1", 8);
        const borrowAmount = parseUnits("5000", 6);
...
describe("profitability checks skipped when no pricers", () => {
    ...
    encoderTest.sequential("should execute liquidation without pricers (profitability check disabled)", async ({ encoder }) => {
        const { client } = encoder;
        const collateralAmount = parseUnits("0.1", 8);
        const borrowAmount = parseUnits("5000", 6);
        const _marketParams = await readContract(encoder.client, {
