...
import { encodePacked, fromHex, keccak256, maxUint128, maxUint256, parseUnits, toHex, } from "viem";
...
// ── Helper functions ──
async function setupPositionOnBase(client, marketParams, collateralAmount, borrowAmount) {
...
describe("Base chain flash loan liquidation - full path test", () => {
    ...
    baseTest.sequential("full flash loan liquidation path on Base (WETH/USDC market)", async ({ encoder, client }) => {
        ...
        const marketId = await setupPositionOnBase(client, marketParams, collateralAmount, borrowAmount);
        ...
        encoder.balancerFlashLoan(BALANCER_VAULT_ADDRESS, [{ asset: USDC_BASE, amount: borrowAmount }], []);
