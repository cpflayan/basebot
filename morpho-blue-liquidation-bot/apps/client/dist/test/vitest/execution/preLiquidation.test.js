...
import { PreLiquidationPosition } from "@morpho-org/blue-sdk";
...
async function setupPreLiquidationPosition(client) {
    const marketId = wbtcUSDT;
...
describe("execute pre-liquidation", () => {
    ...
    preLiquidationTest.sequential("should execute pre-liquidation on WBTC/USDT market", async ({ encoder }) => {
    ...
    preLiquidationTest.sequential("should skip pre-liquidation when no pricers configured (profitability checks disabled)", async ({ encoder }) => {
        const { client } = encoder;
        const { marketParams, preLiqPosition } = await setupPreLiquidationPosition(client);
        const mockDataProvider = new MockDataProvider();
        mockDataProvider.setPreLiquidatablePositions([preLiqPosition]);
