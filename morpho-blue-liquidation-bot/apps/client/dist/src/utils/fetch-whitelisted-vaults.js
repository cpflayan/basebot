...
`;
export async function fetchWhitelistedVaults(chainId) {
    ...
    for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
        try {
            ...
            if (items.length === 0) {
                console.warn(`[fetchWhitelistedVaults] ⚠️ chainId=${chainId} 返回 0 個 vault，` +
            ...
            if (attempt < MAX_RETRIES) {
                console.warn(`[fetchWhitelistedVaults] 嘗試 ${attempt + 1} 失敗，重試...`, e.message);
                await new Promise((r) => setTimeout(r, 1000 * (attempt + 1))); // 退避
                continue;
            }
            console.error(`[fetchWhitelistedVaults] 所有重試失敗:`, e.message);
            return [];
        }
    }
    return [];
