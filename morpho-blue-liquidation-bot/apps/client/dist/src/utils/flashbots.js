...
export var Flashbots;
(function (Flashbots) {
    let nextId = 0;
    Flashbots.FLASHBOTS_RELAY = "https://relay.flashbots.net";
    /**
    ...
    async function signBundle(bundle) {
        ...
        for (const { transaction, client } of bundle) {
            ...
            const nonce = transaction.nonce ?? nonces[address] ?? (await getTransactionCount(client, { address }));
    ...
    Flashbots.signBundle = signBundle;
    /**
     ...
     * ```
     */
    async function sendRawBundle(txs, targetBlockNumber, account) {
        ...
        const response = await fetch(Flashbots.FLASHBOTS_RELAY, {
            method: "POST",
            headers: {
    ...
    Flashbots.sendRawBundle = sendRawBundle;
})(Flashbots || (Flashbots = {}));
