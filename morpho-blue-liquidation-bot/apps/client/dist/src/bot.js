...
import { erc20Abi, formatUnits, getAddress, maxUint256, parseUnits, } from "viem";
import { getBlockNumber, getGasPrice, readContract, simulateCalls, writeContract, } from "viem/actions";
...
export class LiquidationBot {
    ...
    async run() {
        ...
        const { liquidatablePositions, preLiquidatablePositions } = await this.dataProvider.fetchLiquidatablePositions(this.client, this.coveredMarkets);
        await Promise.all([
    ...
    async liquidate(position) {
        ...
        if (!(await this.convertCollateralToLoan(marketParams, this.decreaseSeizableCollateral(seizableCollateral, badDebtPosition), encoder)))
            return;
        encoder.erc20Approve(marketParams.loanToken, this.chainAddresses.morpho, maxUint256);
        encoder.morphoBlueLiquidate(this.chainAddresses.morpho, {
            loanToken: marketParams.loanToken,
            collateralToken: marketParams.collateralToken,
            oracle: marketParams.oracle,
            irm: marketParams.irm,
            lltv: BigInt(marketParams.lltv),
        }, position.user, seizableCollateral, 0n, encoder.flush());
        ...
        try {
            ...
            if (success)
                console.log(`${this.logTag}Liquidated ${position.user} on ${MarketUtils.getMarketId(marketParams)}`);
            else
                console.log(`${this.logTag}ℹ️ Skipped ${position.user} on ${MarketUtils.getMarketId(marketParams)} (not profitable)`);
        }
        catch (error) {
            console.error(`${this.logTag}Failed to liquidate ${position.user} on ${MarketUtils.getMarketId(marketParams)}`, error);
        }
    }
    /**
     ...
     */
    async liquidateWithFlashLoan(position, badDebtPosition) {
        const marketParams = position.market.params;
        const seizableCollateral = position.seizableCollateral ?? 0n;
        const { client, executorAddress } = this;
        const encoder = new LiquidationEncoder(executorAddress, client);
        ...
        if (collateralValue !== undefined) {
            if (collateralValue < flashLoanAmount) {
                console.log(`${this.logTag}[FlashLoan] Skip ${position.user}: bad debt — collateral value (${collateralValue}) < debt (${flashLoanAmount})`);
                return;
            }
            ...
            const seizableValue = totalCollateral > 0n ? (collateralValue * seizableCollateral) / totalCollateral : 0n;
            if (seizableValue < flashLoanAmount) {
                console.log(`${this.logTag}[FlashLoan] Skip ${position.user}: seizable value (${seizableValue}) < debt (${flashLoanAmount}) — not enough collateral to seize for profit`);
                return;
            }
        }
        ...
        const swapSuccess = await this.convertCollateralToLoan(marketParams, this.decreaseSeizableCollateral(seizableCollateral, badDebtPosition), tempEncoder);
        if (!swapSuccess) {
            console.log(`${this.logTag}No DEX route for ${marketParams.collateralToken} -> ${marketParams.loanToken}, skipping flash loan liquidation`);
            return;
        }
        ...
        callbackEncoder.morphoBlueLiquidate(this.chainAddresses.morpho, {
            loanToken: marketParams.loanToken,
            collateralToken: marketParams.collateralToken,
            oracle: marketParams.oracle,
            irm: marketParams.irm,
            lltv: BigInt(marketParams.lltv),
        }, position.user, seizableCollateral, 0n, callbackEncoder.flush());
        ...
        encoder.balancerFlashLoan(BALANCER_VAULT_ADDRESS, [{ asset: marketParams.loanToken, amount: flashLoanAmount }], callbackCalls);
        const calls = encoder.flush();
        try {
            const success = await this.handleFlashLoanSimulationAndExec(encoder, calls, marketParams, badDebtPosition, flashLoanAmount);
            if (success)
                console.log(`${this.logTag}[FlashLoan] Liquidated ${position.user} on ${MarketUtils.getMarketId(marketParams)}`);
            else
                console.log(`${this.logTag}[FlashLoan] Skipped ${position.user} on ${MarketUtils.getMarketId(marketParams)} (not profitable)`);
        }
        catch (error) {
            console.error(`${this.logTag}[FlashLoan] Failed to liquidate ${position.user} on ${MarketUtils.getMarketId(marketParams)}`, error);
        }
    }
    async preLiquidate(position) {
        const marketParams = position.market.params;
        const seizableCollateral = this.decreaseSeizableCollateral(position.seizableCollateral ?? 0n, false);
        if (!this.checkCooldown(MarketUtils.getMarketId(marketParams), position.user))
            return;
        const { client, executorAddress } = this;
        const encoder = new LiquidationEncoder(executorAddress, client);
        ...
        encoder.preLiquidate(position.preLiquidation, position.user, seizableCollateral, 0n, encoder.flush());
        encoder.erc20Skim(marketParams.loanToken, this.treasuryAddress);
        const calls = encoder.flush();
        try {
            const success = await this.handleTx(encoder, calls, marketParams, false);
            if (success)
                console.log(`${this.logTag}Pre-liquidated ${position.user} on ${MarketUtils.getMarketId(marketParams)}`);
            else
                console.log(`${this.logTag}ℹ️ Skipped ${position.user} on ${MarketUtils.getMarketId(marketParams)} (not profitable)`);
        }
        catch (error) {
            console.error(`${this.logTag}Failed to pre-liquidate ${position.user} on ${MarketUtils.getMarketId(marketParams)}`, error);
        }
    }
    async handleTx(encoder, calls, marketParams, badDebtPosition, flashLoanAmount) {
        const functionData = {
            abi: executorAbi,
        ...
        if (!(await this.checkProfit(marketParams.loanToken, {
            beforeTx: results[0].result,
            afterTx: results[2].result,
        }, {
            used: results[1].gasUsed,
            price: gasPrice,
        }, badDebtPosition, flashLoanAmount)))
            return false;
        // TX EXECUTION
        if (this.flashbotAccount) {
            ...
            await Flashbots.sendRawBundle(signedBundle, (await getBlockNumber(this.client)) + 1n, this.flashbotAccount);
            return true;
        }
        else {
     ...
     */
    async handleFlashLoanSimulationAndExec(encoder, calls, marketParams, badDebtPosition, flashLoanAmount) {
        const functionData = {
            abi: executorAbi,
            functionName: "exec_606BaXt",
            args: [calls],
        ...
        if (simulatedProfit < minProfitThreshold) {
            console.warn(`${this.logTag}[FlashLoan] Simulated profit (${simulatedProfit}) below threshold (${minProfitThreshold}), ` +
    ...
    async convertCollateralToLoan(marketParams, seizableCollateral, encoder) {
        let toConvert = {
    ...
    async price(asset, amount, pricers) {
        let price = undefined;
        ...
        if (price === undefined)
            return undefined;
        const decimals = asset === this.wNative
            ? 18
    ...
    async checkProfit(loanAsset, loanAssetBalance, gas, badDebtPosition, flashLoanAmount) {
        ...
        if (this.pricers === undefined || this.pricers.length === 0) {
            console.error(`${this.logTag}⛔ No pricers configured — refusing to execute trade (cannot verify profitability). ` +
    ...
    decreaseSeizableCollateral(seizableCollateral, badDebtPosition) {
        if (badDebtPosition)
            return seizableCollateral;
        const liquidationBufferBps = chainConfigs[this.chainId]?.options.liquidationBufferBps ?? DEFAULT_LIQUIDATION_BUFFER_BPS;
        return wMulDown(seizableCollateral, WAD - parseUnits(liquidationBufferBps.toString(), 14));
    }
    checkCooldown(marketId, account) {
        if (this.positionLiquidationCooldownMechanism !== undefined &&
    ...
    async fetchMarkets() {
        if (!this.marketsFetchingCooldownMechanism.isFetchingReady())
            return;
        ...
        const whitelistedMarketsFromVaults = await this.dataProvider.fetchMarkets(this.client, vaultWhitelist);
