...
export class MorphoApiDataProvider {
    async fetchMarkets(client, vaults) {
        try {
            const vaultMarkets = await Promise.all(vaults.map(async (vault) => this.fetchVaultMarkets(client, vault)));
    ...
    async fetchLiquidatablePositions(client, marketIds) {
        try {
            ...
            const positions = allPositions.filter((position) => position.market.uniqueKey !== undefined &&
                position.market.oracle !== null &&
                position.state !== null);
            if (positions.length === 0)
                return { liquidatablePositions: [], preLiquidatablePositions: [] };
            const marketResults = await Promise.allSettled([...marketIds].map(async (marketId) => {
                ...
                return [marketId, market.accrueInterest(timestamp)];
            }));
            const marketsMap = new Map(marketResults
            ...
            return {
                liquidatablePositions: accruedPositions.filter((position) => position.seizableCollateral !== undefined),
    ...
    async fetchVaultMarkets(client, vaultAddress) {
        try {
            ...
            console.error(`[Chain ${client.chain.id}] Error fetching vault markets for ${vaultAddress}:`, error);
            return [];
        }
    }
}
