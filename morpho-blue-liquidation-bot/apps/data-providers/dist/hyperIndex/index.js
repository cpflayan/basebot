...
export class HyperIndexDataProvider {
    graphqlClient;
    url;
    selfhost;
    indexerProcess;
    ...
    async fetchMarkets(client, vaults) {
        try {
            const vaultIds = vaults.map((v) => `${client.chain.id}-${v.toLowerCase()}`);
            const response = await this.graphqlClient.request(GET_VAULT_MARKETS, {
                vaultIds,
            });
            const marketIds = response.Vault.flatMap((vault) => vault.withdrawQueue.filter((item) => item.market_id != null).map((item) => item.market_id));
    ...
    async fetchLiquidatablePositions(client, marketIds) {
        try {
            ...
            const fetchPositions = async () => {
                let offset = 0;
                while (true) {
                    const page = await this.graphqlClient.request(GET_POSITIONS, {
                        marketIds: indexedMarketIds,
                        limit: POSITIONS_PAGE_SIZE,
                        offset,
                    });
                    allPositions.push(...page.Position);
                    if (page.Position.length < POSITIONS_PAGE_SIZE)
                        break;
            ...
            const [, marketsResponse, preLiqContracts] = await Promise.all([
                fetchPositions(),
                this.graphqlClient.request(GET_MARKETS, {
                    marketIds: indexedMarketIds,
                }),
                this.graphqlClient.request(GET_PRELIQUIDATION_CONTRACTS, {
                    marketIds: indexedMarketIds,
                }),
            ]);
            ...
            const preLiqAddresses = preLiqContracts.PreLiquidationContract.map((plc) => plc.address.toLowerCase());
            let authorizations = { Authorization: [] };
            if (preLiqAddresses.length > 0) {
                authorizations = await this.graphqlClient.request(GET_AUTHORIZATIONS, {
            ...
            await Promise.all([...oracleAddresses].map(async (oracle) => {
                try {
            ...
            const preLiqContractsByMarket = new Map();
            for (const plc of preLiqContracts.PreLiquidationContract) {
            ...
            const preLiqCandidates = [];
            for (const p of allPositions) {
                if (!indexedMarketIds.includes(p.market_id))
                    continue;
                const market = marketsMap.get(p.market_id);
                if (!market)
                    continue;
            ...
            preLiqCandidates.sort((a, b) => (a.seizableCollateral ?? 0n) > (b.seizableCollateral ?? 0n) ? -1 : 1);
            const seenUsers = new Set();
            const preLiquidatablePositions = [];
            ...
            console.error(`[Chain ${client.chain.id}] Error fetching liquidatable positions from HyperIndex:`, error);
            return { liquidatablePositions: [], preLiquidatablePositions: [] };
        }
    }
    async getChainTip(chainId) {
    ...
    async waitForReady() {
        ...
        `);
                ...
                for (const c of chains) {
                    ...
                    if (tip) {
                        ...
                        process.stdout.write(`\x1b[2K${spinner} [HyperIndex] Chain ${c.chain_id}: ${bar} ${pct}% (${c.latest_processed_block.toLocaleString()} / ${tip.toLocaleString()}) [${elapsed}s]\n`);
                    }
                    else {
                        process.stdout.write(`\x1b[2K${spinner} [HyperIndex] Chain ${c.chain_id}: block ${c.latest_processed_block.toLocaleString()} [${elapsed}s]\n`);
                    }
                    lineCount++;
                }
                if (allCaughtUp) {
        ...
        throw new Error(`[HyperIndex] Timed out waiting for backfill at ${this.url} after ${HEALTH_CHECK_TIMEOUT_MS / 1000}s`);
    }
}
