...
import { IncompleteChainRegistryError, RegistryValueAlreadyRegisteredError, UnknownAddressError, UnsupportedChainIdError, } from "./errors.js";
 ...
 * ```
 */
export const getChainAddress = (chainId, label) => {
    const chainAddresses = blueAddresses[chainId];
    if (chainAddresses == null)
        throw new UnsupportedChainIdError(chainId);
...
const _unwrappedTokensMapping = {
    [ChainId.EthMainnet]: {
        [_addressesRegistry[ChainId.EthMainnet].wbIB01]: _addressesRegistry[ChainId.EthMainnet].bIB01,
        [_addressesRegistry[ChainId.EthMainnet].wbC3M]: _addressesRegistry[ChainId.EthMainnet].bC3M,
        [_addressesRegistry[ChainId.EthMainnet].wNative]: NATIVE_ADDRESS,
        [_addressesRegistry[ChainId.EthMainnet].stEth]: NATIVE_ADDRESS,
        [_addressesRegistry[ChainId.EthMainnet].wstEth]: _addressesRegistry[ChainId.EthMainnet].stEth,
        [_addressesRegistry[ChainId.EthMainnet]["stkcvxcrvUSDTWBTCWETH-morpho"]]: _addressesRegistry[ChainId.EthMainnet].crvUSDTWBTCWETH,
        [_addressesRegistry[ChainId.EthMainnet]["stkcvxcrvUSDCWBTCWETH-morpho"]]: _addressesRegistry[ChainId.EthMainnet].crvUSDCWBTCWETH,
        [_addressesRegistry[ChainId.EthMainnet]["stkcvxcrvCRVUSDTBTCWSTETH-morpho"]]: _addressesRegistry[ChainId.EthMainnet].crvCRVUSDTBTCWSTETH,
        [_addressesRegistry[ChainId.EthMainnet]["stkcvxTryLSD-morpho"]]: _addressesRegistry[ChainId.EthMainnet].tryLSD,
        [_addressesRegistry[ChainId.EthMainnet]["stkcvxcrvUSDETHCRV-morpho"]]: _addressesRegistry[ChainId.EthMainnet].crvUSDETHCRV,
        [_addressesRegistry[ChainId.EthMainnet]["stkcvx2BTC-f-morpho"]]: _addressesRegistry[ChainId.EthMainnet]["2BTC-f"],
    },
    [ChainId.BaseMainnet]: {
        [_addressesRegistry[ChainId.BaseMainnet].wNative]: NATIVE_ADDRESS,
        [_addressesRegistry[ChainId.BaseMainnet].verUsdc]: _addressesRegistry[ChainId.BaseMainnet].usdc,
        [_addressesRegistry[ChainId.BaseMainnet].testUsdc]: _addressesRegistry[ChainId.BaseMainnet].usdc,
    },
    [ChainId.PolygonMainnet]: {
...
entries(permissionedCoinbaseTokens).forEach(([chainId, tokens]) => {
    tokens.forEach((token) => (permissionedWrapperTokens[chainId] ??= new Set()).add(token));
});
...
export function registerCustomAddresses({ unwrappedTokens, addresses: customAddresses, deployments: customDeployments, } = {}) {
    ...
    if (unwrappedTokens) {
        unwrappedTokensMapping = deepFreeze(mergeRegistry({
