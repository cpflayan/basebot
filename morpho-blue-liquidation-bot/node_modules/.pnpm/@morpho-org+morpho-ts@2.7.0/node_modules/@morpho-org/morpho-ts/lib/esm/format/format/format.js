...
const _formatShort = (bi, decimals, formatOptions) => {
    ...
    if (params) {
        return (_applyOptions(insertString(stringValue, -((params.power ?? params.minDecimals) + decimals), ".", "0"), formatOptions) + params.symbol);
    }
    if (formatOptions.smallValuesWithCommas) {
        return _formatCommas(bi, decimals, formatOptions);
    }
    return _applyOptions(decimals ? insertString(stringValue, -decimals, ".", "0") : stringValue, formatOptions);
};
// biome-ignore lint/complexity/useMaxParams: TODO refactor to ≤2 params
const _formatCommas = (bi, decimals, formatOptions) => {
    ...
    return _applyOptions(wholePart.split("").reduce(
    // biome-ignore lint/complexity/useMaxParams: TODO refactor to ≤2 params
    (formattedNumber, digit, i, arr) => `${formattedNumber}${!i || (arr.length - i) % 3 ? "" : ","}${digit}`, "") + (decimalPart ? `.${decimalPart}` : ""), formatOptions);
};
// biome-ignore lint/complexity/useMaxParams: TODO refactor to ≤2 params
const _formatNumber = (bi, decimals, formatOptions) => {
    if (decimals === 0)
        return _applyOptions(bi.toString(), formatOptions);
    return _applyOptions(insertString(bi.toString(), -decimals, ".", "0"), formatOptions);
};
 ...
 */
export class CommonFormatter extends BaseFormatter {
    /**
 ...
 */
export function createFormat(defaultOptions = {}, customFormatters = {}) {
    const formatters = {
        get hex() {
