import { deepFreeze, entries, } from "@morpho-org/morpho-ts";
...
export function registerCustomAddresses({ unwrappedTokens, addresses: customAddresses, deployments: customDeployments, } = {}) {
    ...
    if (customAddresses)
        addresses = addressesRegistry = deepFreeze(mergeWith({}, addressesRegistry, customAddresses, customizer("address")));
    if (customDeployments)
        deployments = deepFreeze(mergeWith({}, deployments, customDeployments, customizer("deployment")));
    if (unwrappedTokens)
        unwrappedTokensMapping = deepFreeze(mergeWith({}, unwrappedTokensMapping, unwrappedTokens, customizer("unwrapped token")));
}
