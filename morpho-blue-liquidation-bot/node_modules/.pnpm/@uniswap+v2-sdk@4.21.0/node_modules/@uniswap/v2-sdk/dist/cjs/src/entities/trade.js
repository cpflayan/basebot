...
class Trade {
    ...
    static exactIn(route, amountIn) {
        return new Trade(route, amountIn, sdk_core_1.TradeType.EXACT_INPUT);
    }
    /**
    ...
    static exactOut(route, amountOut) {
        return new Trade(route, amountOut, sdk_core_1.TradeType.EXACT_OUTPUT);
    }
    constructor(route, amount, tradeType) {
        ...
        if (tradeType === sdk_core_1.TradeType.EXACT_INPUT) {
            ...
            this.inputAmount = sdk_core_1.CurrencyAmount.fromFractionalAmount(route.input, amount.numerator, amount.denominator);
            this.outputAmount = sdk_core_1.CurrencyAmount.fromFractionalAmount(route.output, tokenAmounts[tokenAmounts.length - 1].numerator, tokenAmounts[tokenAmounts.length - 1].denominator);
        }
        else {
            ...
            this.inputAmount = sdk_core_1.CurrencyAmount.fromFractionalAmount(route.input, tokenAmounts[0].numerator, tokenAmounts[0].denominator);
            this.outputAmount = sdk_core_1.CurrencyAmount.fromFractionalAmount(route.output, amount.numerator, amount.denominator);
        }
        this.executionPrice = new sdk_core_1.Price(this.inputAmount.currency, this.outputAmount.currency, this.inputAmount.quotient, this.outputAmount.quotient);
        this.priceImpact = (0, sdk_core_1.computePriceImpact)(route.midPrice, this.inputAmount, this.outputAmount);
    }
    /**
    ...
    minimumAmountOut(slippageTolerance) {
        ...
        if (this.tradeType === sdk_core_1.TradeType.EXACT_OUTPUT) {
            ...
            const clampedAmount = jsbi_1.default.greaterThan(slippageAdjustedAmountOut, jsbi_1.default.BigInt(0))
                ? slippageAdjustedAmountOut
                : jsbi_1.default.BigInt(0);
            return sdk_core_1.CurrencyAmount.fromRawAmount(this.outputAmount.currency, clampedAmount);
        }
    }
    /**
    ...
    maximumAmountIn(slippageTolerance) {
        ...
        if (this.tradeType === sdk_core_1.TradeType.EXACT_INPUT) {
            ...
            return sdk_core_1.CurrencyAmount.fromRawAmount(this.inputAmount.currency, slippageAdjustedAmountIn);
        }
    }
    /**
    ...
    currentPairs = [], nextAmountIn = currencyAmountIn, bestTrades = []) {
        ...
        for (let i = 0; i < pairs.length; i++) {
            ...
            if (amountOut.currency.equals(tokenOut)) {
                (0, sdk_core_1.sortedInsert)(bestTrades, new Trade(new route_1.Route([...currentPairs, pair], currencyAmountIn.currency, currencyOut), currencyAmountIn, sdk_core_1.TradeType.EXACT_INPUT), maxNumResults, tradeComparator);
            }
    ...
    currentPairs = [], nextAmountOut = currencyAmountOut, bestTrades = []) {
        ...
        for (let i = 0; i < pairs.length; i++) {
            ...
            if (amountIn.currency.equals(tokenIn)) {
                (0, sdk_core_1.sortedInsert)(bestTrades, new Trade(new route_1.Route([pair, ...currentPairs], currencyIn, currencyAmountOut.currency), currencyAmountOut, sdk_core_1.TradeType.EXACT_OUTPUT), maxNumResults, tradeComparator);
            }
            else if (maxHops > 1 && pairs.length > 1) {
                const pairsExcludingThisPair = pairs.slice(0, i).concat(pairs.slice(i + 1, pairs.length));
