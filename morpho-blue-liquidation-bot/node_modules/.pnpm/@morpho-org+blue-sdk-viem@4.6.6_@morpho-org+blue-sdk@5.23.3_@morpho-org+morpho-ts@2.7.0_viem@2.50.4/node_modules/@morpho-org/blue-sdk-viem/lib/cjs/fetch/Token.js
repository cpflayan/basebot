...
const GetToken_js_1 = require("../queries/GetToken.js");
...
async function fetchToken(address, client, { deployless = true, ...parameters } = {}) {
    ...
    if (deployless) {
        try {
            ...
            const token = await (0, actions_1.readContract)(client, {
                ...parameters,
                abi: GetToken_js_1.abi,
                code: GetToken_js_1.code,
                functionName: "query",
            ...
            if (isWstEth && stEth != null)
                return new blue_sdk_1.ExchangeRateWrappedToken(metadata, stEth, token.stEthPerWstEth);
            const unwrapToken = (0, blue_sdk_1.getUnwrappedToken)(address, parameters.chainId);
            if (unwrapToken)
                return new blue_sdk_1.ConstantWrappedToken(metadata, unwrapToken, token.decimals);
            return new blue_sdk_1.Token(metadata);
        }
        catch (error) {
            if (deployless === "force")
                throw error;
    ...
    const [decimals, symbol, name, eip5267Domain] = await Promise.all([
        ...
        (0, actions_1.readContract)(client, {
            ...parameters,
            address,
            abi: abis_js_1.erc5267Abi,
            functionName: "eip712Domain",
        })
            .then(([fields, 
        // biome-ignore lint/nursery/noShadow: TODO rename to avoid shadowing
        name, version, chainId, verifyingContract, salt, extensions,]) => new blue_sdk_1.Eip5267Domain({
            fields,
            name,
            version,
            chainId,
