import { getChainAddresses, Market, MarketParams, } from "@morpho-org/blue-sdk";
...
export async function fetchMarket(id, client, { deployless = true, ...parameters } = {}) {
    parameters.chainId ??= await getChainId(client);
    const { morpho, adaptiveCurveIrm } = getChainAddresses(parameters.chainId);
    if (deployless) {
        try {
            const { marketParams, market: { totalSupplyAssets, totalSupplyShares, totalBorrowAssets, totalBorrowShares, lastUpdate, fee, }, hasPrice, price, rateAtTarget, } = await readContract(client, {
                ...parameters,
                abi,
                code,
                functionName: "query",
