...
import { InvalidPermitDomainChainIdError, InvalidPermitDomainVerifyingContractError, UnsupportedPermitDomainExtensionsError, } from "../error.js";
const permitTypes = {
    Permit: [
 ...
 */
export const getPermitTypedData = ({ deadline, owner, nonce, spender, erc20, allowance }, chainId) => {
    const { usdc, eurc } = getChainAddresses(chainId);
    const eip5267Domain = erc20.eip5267Domain;
    if (eip5267Domain?.extensions.length) {
        throw new UnsupportedPermitDomainExtensionsError(erc20.address, eip5267Domain.extensions);
    }
    const domain = eip5267Domain?.eip712Domain;
    if (domain != null) {
        if (domain.chainId !== chainId) {
            throw new InvalidPermitDomainChainIdError(erc20.address, chainId, domain.chainId);
        }
        if (domain.verifyingContract == null ||
            !isAddressEqual(domain.verifyingContract, erc20.address)) {
            throw new InvalidPermitDomainVerifyingContractError(erc20.address, domain.verifyingContract);
        }
    }
...
export const getDaiPermitTypedData = ({ deadline, owner, nonce, spender, allowance }, chainId) => {
