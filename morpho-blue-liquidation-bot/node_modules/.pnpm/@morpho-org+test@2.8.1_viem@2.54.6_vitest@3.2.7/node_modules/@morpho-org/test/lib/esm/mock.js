...
import { createClient, custom, decodeFunctionData, encodeFunctionResult, toFunctionSelector, toFunctionSignature, } from "viem";
 ...
 * ```
 */
export function createMockClient(chain) {
 ...
 * ```
 */
export function mockRead(handle, options) {
    ...
    const fnAbiItems = options.abi.filter((item) => item.type === "function" && item.name === options.functionName);
    ...
    if (registered === 0)
        throw new Error(`[mockRead] options.result does not match any return-type shape of overloads of ${String(options.functionName)} (${fnAbiItems.length} overload(s) tried)`);
}
/**
 ...
 * ```
 */
export function expectReadCall(handle, match) {
    ...
    const selectors = new Set(match.abi
        .filter((item) => item.type === "function" && item.name === match.functionName)
    ...
    return handle.request.mock.calls.flatMap(([call]) => {
        ...
        if (typeof tx.data !== "string")
            return [];
        if (!selectors.has(tx.data.slice(0, 10).toLowerCase()))
            return [];
