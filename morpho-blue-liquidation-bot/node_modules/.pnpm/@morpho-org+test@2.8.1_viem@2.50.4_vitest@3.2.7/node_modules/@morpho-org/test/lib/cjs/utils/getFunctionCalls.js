 ...
 * ```
 */
async function getFunctionCalls(client, { txHash, abi, contract, functionName }) {
    const trace = await client.request({
        method: "ots_traceTransaction",
        params: [txHash],
    });
    const target = (0, viem_1.getAddress)(contract);
    const calls = [];
    const walk = (node) => {
        if (Array.isArray(node))
            return node.forEach(walk);
        if (node?.to && node?.input) {
            if ((0, viem_1.getAddress)(node.to) === target) {
                try {
                    const call = (0, viem_1.decodeFunctionData)({
                        abi,
                        data: node.input,
                    });
