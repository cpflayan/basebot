...
async function pollChain(setup: (typeof CHAIN_SETUPS)[number]): Promise<void> {
  ...
  const balanceWei = await client.getBalance({ address: executorAddress as `0x${string}` });
  ...
  console.log(
    `[circuit-breaker] ${setup.chain.name} executor 餘額=${balance.toFixed(6)} 峰值=${peakBalance.toFixed(6)} 回撤=${drawdownPct.toFixed(2)}%`,
  );
