...
import { MORPHO_BLUE_ADDRESS, CREATE_MARKET_EVENT, type ChainSetup } from "../shared/chains.js";
...
// SECURITY: Token 黑名單 — 涉及這些 token 的市場直接跳過
const TOKEN_BLACKLIST = new Set([
  "0x35E5dB674D8e93a03d814FA0ADa70731efe8a4b9".toLowerCase(), // USR (Resolv USD) on Base
]);
...
function loadLastBlock(chainId: number): { lastBlock: bigint; blockHash: string } | null {
...
function saveLastBlock(chainId: number, block: bigint, blockHash: string): void {
  const p = path.join(CHECKPOINT_DIR, `watch-checkpoint.${chainId}.json`);
  try {
...
export function watchChain(chainSetup: ChainSetup): void {
  ...
  async function poll(): Promise<void> {
    try {
      ...
      if (needsReorgCheck && lastBlock !== null && lastBlockHash) {
        ...
        try {
          ...
          if (savedBlock.hash && savedBlock.hash !== lastBlockHash) {
            console.warn(
              `[discovery] ⚠️ 鏈重組檢測：checkpoint block ${lastBlock} hash 不匹配 ` +
      ...
      while (fromBlock <= toBlock) {
        ...
        for (const log of logs) {
          try {
            ...
            // 黑名單過濾
            const loanLower = marketParams.loanToken.toLowerCase();
            const collLower = marketParams.collateralToken.toLowerCase();
            if (TOKEN_BLACKLIST.has(loanLower) || TOKEN_BLACKLIST.has(collLower)) {
              console.log(`[discovery] ⛔ 跳過市場 ${id.slice(0, 10)}... — 涉及黑名單 token`);
              continue;
            }
            ...
            await notify(
              `${statusEmoji} [${chainSetup.chain.name}] 新市場 ${id.slice(0, 10)}...\n` +
                `分數: ${result.score}/100\n` +
                `${result.notes.join("\n")}`,
            );
          } catch (e) {
      ...
      console.error(`[discovery] ${chainSetup.chain.name} polling 錯誤:`, (e as Error).message);
    }
