...
import type { ChainSetup } from "../shared/chains.js";
...
const SAFE_PROXY_ADDRESSES: Record<number, Set<string>> = {
  8453: new Set([
  ...
  1: new Set(([] as string[]).map((a) => a.toLowerCase())),
  42161: new Set(([] as string[]).map((a) => a.toLowerCase())),
};
...
(function checkProxyWhitelistFreshness() {
  ...
  if (daysSince > 90) {
    console.warn(
      `[safety] ⚠️ 代理白名單已 ${daysSince} 天未審查（上次: ${PROXY_WHITELIST_LAST_REVIEWED}）。` +
  ...
  if (chainlinkFactory) {
    try {
      const isChainlinkOracle = await client.readContract({
        address: chainlinkFactory as `0x${string}`,
  ...
  if (pythFactory) {
    try {
      const isPythOracle = await client.readContract({
        address: pythFactory as `0x${string}`,
...
async function checkChainlinkFreshness(
  client: ReturnType<typeof createPublicClient>,
  oracleAddress: string,
): Promise<string> {
  try {
...
async function checkPythFreshness(
  client: ReturnType<typeof createPublicClient>,
  oracleAddress: string,
): Promise<string> {
...
async function detectProxy(
  ...
  tokenAddress: string,
  label: string,
): Promise<{ isProxy: boolean; note: string }> {
  try {
 ...
 */
async function smokeTestTransfer(
  client: ReturnType<typeof createPublicClient>,
  tokenAddress: string,
  label: string,
  ...
  try {
    ...
    const testSender = "0x000000000000000000000000000000000000dEaD" as `0x${string}`;
...
// SECURITY (M2): 鏈上流動性檢查 — 直接查詢 + 多跳路由檢測 + 1inch API 備援
// 鏈上：Uniswap V3 + Aerodrome；備援：1inch Swap API（覆蓋更多交易對）
// 策略：先查直接池，沒有則查多跳路由，最後用 1inch API 兜底

const ONE_INCH_API_BASE = "https://api.1inch.dev";
const oneInchApiKey: string | undefined = process.env.ONE_INCH_SWAP_API_KEY;

async function checkDexLiquidity(
  chainSetup: ChainSetup,
  sellToken: string,
  buyToken: string,
): Promise<{ liquid: boolean; note: string; estimatedUsd: number; softFail: boolean }> {
  // 跳過零地址（異常市場）
  ...
  const directResults: { liquid: boolean; note: string; estimatedUsd: number }[] = [];
  ...
  if (chainSetup.chainId === 8453) {
    ...
    if (uniResult) directResults.push(uniResult);

    const aeroResult = await checkAerodromeLiquidity(chainSetup, sellToken, buyToken);
    if (aeroResult) directResults.push(aeroResult);
  }
  ...
  if (chainSetup.chainId === 8453 && !sellIsIntermediary && !buyIsIntermediary) {
    ...
    for (const intermediary of INTERMEDIARIES) {
      // 跳过与自身相同的中间代币
      if (sellToken.toLowerCase() === intermediary) { sellHasRoute = true; continue; }
      if (buyToken.toLowerCase() === intermediary) { buyHasRoute = true; continue; }

      if (!sellHasRoute) {
        const r = await checkUniswapV3Liquidity(chainSetup, sellToken, intermediary);
        if (r?.liquid) sellHasRoute = true;
      }
      if (!sellHasRoute) {
      ...
      if (sellHasRoute && buyHasRoute) break;
    }
  ...
  // 3. 1inch API 兜底：鏈上檢查失敗時，用 1inch 聚合器確認流動性
  if (oneInchApiKey) {
    const oneInchResult = await checkOneInchLiquidity(chainSetup.chainId, sellToken, buyToken);
    if (oneInchResult) {
      return { ...oneInchResult, softFail: false };
    }
  }
...
// 1inch Swap API 流動性兜底檢查
// 當鏈上 DEX 檢查失敗時，用 1inch 聚合器確認是否存在路由
async function checkOneInchLiquidity(
  chainId: number,
  sellToken: string,
  buyToken: string,
): Promise<{ liquid: boolean; note: string; estimatedUsd: number } | null> {
  if (!oneInchApiKey) return null;

  try {
    // 用 quote 端點檢查路由可用性（不需要 from 地址，更輕量）
    const url = new URL(`/swap/v6.1/${chainId}/quote`, ONE_INCH_API_BASE);
    url.searchParams.set("src", sellToken);
    url.searchParams.set("dst", buyToken);
    // 用 100 单位（考虑 decimals 后面调整）作为测试金额
    url.searchParams.set("amount", "1000000000000000000"); // 1e18 as default

    console.log(`[safety] 1inch API 查詢: ${sellToken.slice(0, 10)}→${buyToken.slice(0, 10)}`);
    ...
    if (!res.ok) {
      console.log(`[safety] 1inch API 返回 ${res.status}，無路由`);
      return { liquid: false, note: `1inch API 無可用路由 (HTTP ${res.status})`, estimatedUsd: 0 };
    }

    const data = (await res.json()) as { dstAmount?: string; protocols?: unknown[][][] };

    if (data.dstAmount && BigInt(data.dstAmount) > 0n) {
      console.log(`[safety] 1inch 確認有路由，dstAmount=${data.dstAmount}`);
      return {
        liquid: true,
        note: `1inch API 確認有流動性路由（dstAmount=${data.dstAmount}）`,
        estimatedUsd: 5000, // 保守估計
      };
    }

    return { liquid: false, note: "1inch API 返回 0 輸出金額", estimatedUsd: 0 };
  } catch (e) {
    console.warn(`[safety] 1inch API 查詢失敗:`, (e as Error).message);
    return null;
  }
}

// Uniswap V3 流動性直接查詢（不依賴聚合器 API）
async function checkUniswapV3Liquidity(
  chainSetup: ChainSetup,
  tokenA: string,
  tokenB: string,
): Promise<{ liquid: boolean; note: string; estimatedUsd: number } | null> {
  ...
  try {
    ...
    console.warn(`[safety] Uniswap V3 流動性查詢失敗:`, (e as Error).message);
    return null;
  }
}

// Aerodrome 流動性查詢（Base 鏈最大 DEX，Velodrome V2 fork）
async function checkAerodromeLiquidity(
  chainSetup: ChainSetup,
  tokenA: string,
  tokenB: string,
): Promise<{ liquid: boolean; note: string; estimatedUsd: number } | null> {
  ...
  try {
    ...
    for (const stable of [false, true]) {
      try {
        const pool = await client.readContract({
          ...
          abi: FACTORY_ABI,
          functionName: "getPool",
          args: [tokenA as `0x${string}`, tokenB as `0x${string}`, stable],
        });
    ...
    console.warn(`[safety] Aerodrome 流動性查詢失敗:`, (e as Error).message);
    return null;
  }
}
...
export async function runSafetyChecks(input: SafetyCheckInput): Promise<SafetyCheckResult> {
  ...
  for (const [label, token] of [
    ...
    // 3a. decimals 可读
    try {
      await client.readContract({ address: token as `0x${string}`, abi: erc20Abi, functionName: "decimals" });
    } catch {
