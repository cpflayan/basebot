...
function calcHealthFactor(borrowUsd: number, collateralUsd: number, lltv: number): number {
...
function estimateLiquidationProfit(borrowUsd: number, collateralUsd: number, lltv: number): {
  profitable: boolean;
  maxRepayUsd: number;
  seizedCollateralUsd: number;
  grossProfitUsd: number;
  netProfitUsd: number; // 扣除滑點 + Gas
  isBadDebt: boolean; // 抵押品不足以覆蓋全部債務
  unrecoverableDebtUsd: number; // 無法回收的債務部分
} {
...
async function fetchAllPositions(marketIds: string[]): Promise<any[]> {
  ...
  for (const marketId of marketIds) {
    ...
    while (hasMore) {
      try {
        ...
        console.error(`  抓取 market ${marketId.slice(0, 10)}... skip=${skip} 失敗:`, (e as Error).message);
        hasMore = false;
      }
    }
  }
...
async function loadWhitelist(): Promise<Set<string>> {
  try {
    ...
    const approved = raw
      .filter((m: any) => m.approved === true && m.marketId)
      .map((m: any) => m.marketId.toLowerCase());
    console.log(`✅ 已載入白名單: ${approved.length} 個市場 (from ${WHITELIST_PATH})\n`);
    return new Set(approved);
  } catch (e) {
    console.warn(`⚠️  讀取白名單失敗: ${(e as Error).message}，將掃描所有市場\n`);
    return new Set();
  }
}
...
async function main() {
  ...
  // 注意：Morpho GraphQL API 不支持 marketUniqueKey_in 過濾 markets
  // 必須抓取全部市場，本地過濾白名單
  ...
  while (true) {
    ...
    if (allMarkets.length % 500 === 0) console.log(`  已抓取 ${allMarkets.length}/${totalMarkets} 個市場`);
    if (allMarkets.length >= totalMarkets || markets.length < batchSize) break;
    skip += batchSize;
  }
  ...
  if (useWhitelist) {
    console.log(`  🔒 白名單過濾後: ${filteredMarkets.length}/${whitelist.size} 個市場`);
    
    // 檢查是否有白名單市場不在 API 中
    const foundIds = new Set(allMarkets.map((m) => m.marketId.toLowerCase()));
    const missingIds = Array.from(whitelist).filter((id) => !foundIds.has(id));
    if (missingIds.length > 0) {
      console.log(`  ⚠️  ${missingIds.length} 個白名單市場不在 API 中（可能未索引或已下架）`);
    }
  }
  console.log("");
  ...
  for (const m of filteredMarkets) marketMap.set(m.marketId, m);
  ...
  interface Target {
    ...
    profit: ReturnType<typeof estimateLiquidationProfit>;
  }

  const badDebt: Target[] = []; // HF<1 且抵押品 <$100（壞賬，不可清算）
  const profitableLiquidations: Target[] = []; // HF<1 且淨利潤>0（真正可清算）
  const unprofitableUnderwater: Target[] = []; // HF<1 但淨利潤<=0（抵押品不足，清算會虧錢）
  const atRisk: Target[] = []; // HF 1~1.05（接近清算）
  const safe: Target[] = []; // HF > 1.05
  let skipped = 0; // 粉塵倉位
  ...
  for (const t of profitableLiquidations.slice(0, 30)) {
    console.log(
      `  HF=${t.hf.toFixed(4)} | 借款=${fmt$(t.borrowUsd)} | 抵押=${fmt$(t.collateralUsd)} | ` +
  ...
  if (profitableLiquidations.length > 30) console.log(`  ... 還有 ${profitableLiquidations.length - 30} 個`);
  ...
  for (const t of atRisk.slice(0, 30)) {
    ...
    console.log(
      `  HF=${t.hf.toFixed(4)} (距清算 ${distanceToLiq}%) | 借款=${fmt$(t.borrowUsd)} | 抵押=${fmt$(t.collateralUsd)} | ` +
        `${t.loanSymbol}/${t.collateralSymbol} | ${t.user.slice(0, 12)}...`,
    );
  }
  if (atRisk.length > 30) console.log(`  ... 還有 ${atRisk.length - 30} 個`);
